...
<!-- ==================================================================== -->
<xsl:template name="docma_width_from_border">
  <xsl:param name="docma_border_value" />

  <xsl:variable name="docma_b1">
      <xsl:value-of select="normalize-space(substring-before($docma_border_value, ' '))" />
  </xsl:variable>

  <xsl:variable name="docma_b23">
    <xsl:if test="string-length($docma_b1) != 0">
      <xsl:value-of select="normalize-space(substring-after($docma_border_value, ' '))"/>
	</xsl:if>
  </xsl:variable>

  <xsl:variable name="docma_b2">
    <xsl:if test="string-length($docma_b23) != 0">
      <xsl:value-of select="substring-before(concat($docma_b23, ' '), ' ')"/>
	</xsl:if>
  </xsl:variable>

  <xsl:variable name="docma_b3">
    <xsl:if test="string-length($docma_b23) != 0">
      <xsl:value-of select="normalize-space(substring-after($docma_b23, ' '))"/>
	</xsl:if>
  </xsl:variable>

  <xsl:choose>
    <!-- remove the dimension (px, pt, ...) and test if the remaining string is a number -->
    <xsl:when test="(string-length($docma_b1) &gt; 2) and not(string(number(substring($docma_b1, 1, string-length($docma_b1) - 2))) = 'NaN')">
      <xsl:value-of select="$docma_b1"/>
    </xsl:when>
    <xsl:when test="(string-length($docma_b2) &gt; 2) and not(string(number(substring($docma_b2, 1, string-length($docma_b2) - 2))) = 'NaN')">
      <xsl:value-of select="$docma_b2"/>
    </xsl:when>
    <xsl:when test="(string-length($docma_b3) &gt; 2) and not(string(number(substring($docma_b3, 1, string-length($docma_b3) - 2))) = 'NaN')">
      <xsl:value-of select="$docma_b3"/>
    </xsl:when>
  </xsl:choose>
</xsl:template>
...
<xsl:template name="table.frame">
  ...
  <xsl:variable name="style_value">
    <xsl:value-of select="concat(translate(@style, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), ';')"/>
  </xsl:variable>
  
  <xsl:variable name="docma_border_value">
    <xsl:if test="contains($style_value, 'border:')">
      <xsl:value-of select="normalize-space(substring-before(substring-after($style_value, 'border:'), ';'))"/>
	</xsl:if>
  </xsl:variable>
  
  <xsl:variable name="docma_b_width">
    <xsl:if test="string-length($docma_border_value) != 0">
      <xsl:call-template name="docma_width_from_border" >
        <xsl:with-param name="docma_border_value" select="$docma_border_value" />
      </xsl:call-template>
	</xsl:if>
  </xsl:variable>

  <xsl:variable name="docma_border_width">
    <xsl:choose>
      <xsl:when test="contains($style_value, 'border-width:')">
        <xsl:value-of select="normalize-space(substring-before(substring-after($style_value, 'border-width:'), ';'))" />
      </xsl:when>
      <xsl:when test="string-length($docma_b_width) != 0"><xsl:value-of select="$docma_b_width" /></xsl:when>
      <xsl:when test="boolean(@border)"><xsl:value-of select="@border" />pt</xsl:when>
      <xsl:otherwise><xsl:value-of select="$table.frame.border.thickness"/></xsl:otherwise>
	</xsl:choose>
  </xsl:variable>

  <xsl:variable name="docma_border_color">
    <xsl:choose>
      <xsl:when test="contains($style_value, 'border-color:')">
        <xsl:value-of select="normalize-space(substring-before(substring-after($style_value, 'border-color:'), ';'))" />
      </xsl:when>
      <xsl:when test="contains($docma_border_value, '#')">
        <xsl:text>#</xsl:text><xsl:value-of select="substring-before(concat(substring-after($docma_border_value, '#'), ' '), ' ')" />
      </xsl:when>
      <xsl:when test="contains($docma_border_value, 'black')">black</xsl:when>
      <xsl:when test="contains($docma_border_value, 'white')">white</xsl:when>
      <xsl:when test="contains($docma_border_value, 'blue')">blue</xsl:when>
      <xsl:when test="contains($docma_border_value, 'gray')">gray</xsl:when>
      <xsl:when test="contains($docma_border_value, 'green')">green</xsl:when>
      <xsl:when test="contains($docma_border_value, 'red')">red</xsl:when>
      <xsl:when test="contains($docma_border_value, 'yellow')">yellow</xsl:when>
      <xsl:otherwise><xsl:value-of select="$table.frame.border.color"/></xsl:otherwise>
    </xsl:choose>
  </xsl:variable>

  <xsl:variable name="docma_border_style">
    <xsl:choose>
      <xsl:when test="contains($style_value, 'border-style:')">
        <xsl:value-of select="normalize-space(substring-before(substring-after($style_value, 'border-style:'), ';'))" />
      </xsl:when>
      <xsl:when test="contains($docma_border_value, 'solid')">solid</xsl:when>
      <xsl:when test="contains($docma_border_value, 'dotted')">dotted</xsl:when>
      <xsl:when test="contains($docma_border_value, 'dashed')">dashed</xsl:when>
      <xsl:when test="contains($docma_border_value, 'double')">double</xsl:when>
      <xsl:when test="contains($docma_border_value, 'groove')">groove</xsl:when>
      <xsl:when test="contains($docma_border_value, 'ridge')">ridge</xsl:when>
      <xsl:when test="contains($docma_border_value, 'inset')">inset</xsl:when>
      <xsl:when test="contains($docma_border_value, 'outset')">outset</xsl:when>
      <xsl:when test="contains($docma_border_value, 'none')">none</xsl:when>
      <xsl:otherwise><xsl:value-of select="$table.frame.border.style"/></xsl:otherwise>
	</xsl:choose>
  </xsl:variable>

  <xsl:choose>
    <xsl:when test="$frame='all' or $frame='box' or $frame='border'">
      <xsl:attribute name="border-start-style">
        <xsl:value-of select="$docma_border_style"/>
      </xsl:attribute>
      ...
      <xsl:attribute name="border-start-width">
        <xsl:value-of select="$docma_border_width"/>
      </xsl:attribute>
      ...
      <xsl:attribute name="border-start-color">
        <xsl:value-of select="$docma_border_color"/>
      </xsl:attribute>
    ...
    <xsl:when test="$frame='bottom' or $frame='below'">
    ...
    <xsl:when test="$frame='sides' or $frame='vsides'">
      <xsl:attribute name="border-start-style">
        <xsl:value-of select="$docma_border_style"/>
      </xsl:attribute>
      <xsl:attribute name="border-end-style">
    ...
    <xsl:when test="$frame='top' or $frame='above'">
      <xsl:attribute name="border-start-style">none</xsl:attribute>
      <xsl:attribute name="border-end-style">none</xsl:attribute>
      <xsl:attribute name="border-top-style">
        <xsl:value-of select="$docma_border_style"/>
    ...
    <xsl:when test="$frame='topbot' or $frame='hsides'">
      <xsl:attribute name="border-start-style">none</xsl:attribute>
      <xsl:attribute name="border-end-style">none</xsl:attribute>
      <xsl:attribute name="border-top-style">
        <xsl:value-of select="$docma_border_style"/>
    ...
    <xsl:when test="$frame='none' or $frame='void'">
      <xsl:attribute name="border-start-style">none</xsl:attribute>
      <xsl:attribute name="border-end-style">none</xsl:attribute>
      <xsl:attribute name="border-top-style">none</xsl:attribute>
      <xsl:attribute name="border-bottom-style">none</xsl:attribute>
