...
<xsl:template name="header.navigation">
  ...
  <xsl:variable name="crumbs" select="($docma.breadcrumbs = 'true') and (count($up) &gt; 0)"/>
  <xsl:variable name="row1" select="($navig.showtitles != 0) or $crumbs"/>
    ...
    <div class="navheader">
      <xsl:if test="$row1 or $row2">
        <table width="100%" summary="Navigation header">
          ...
          <xsl:if test="$row2">
            <tr>
              <td style="width:20%;" align="{$direction.align.start}">
              ...
              <td style="width:60%;" align="center">
                <xsl:choose>
                  <xsl:when test="($home != . or $nav.context = 'toc') and ($docma.breadcrumbs != 'true')">
                    <a accesskey="h">
                      <xsl:attribute name="href">
                        <xsl:call-template name="href.target">
                          <xsl:with-param name="object" select="$home"/>
              ...
              <td style="width:20%;" align="{$direction.align.end}">
                <xsl:text>&#160;</xsl:text>
