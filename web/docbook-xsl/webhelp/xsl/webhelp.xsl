<xsl:stylesheet
    ...
    <!-- <xsl:param name="local.l10n.xml" select="document('')"/>  This is set by Docmenta -->
...
<xsl:text>
  ...
  <xsl:template match="/">
    ...
    <xsl:template name="webhelpheader">
        ...
        <div id="header">
            <img id="webhelplogo1" style='margin-right: 0px; padding-right: 0px; padding-top: 0px' align="left"
                 src='../common/images/logo1.png' alt="Company Logo"/>
            <img id="webhelplogo2" style='margin-right: 0px; padding-right: 0px; padding-top: 0px' align="right"
                 src='../common/images/logo2.png' alt="Company Logo"/>
            ...
            <div class="webhelptitle">
                <xsl:if test="count($up) &gt; 0 and generate-id($up) != generate-id($home)">
                    <div class="webhelptitle1">
                        <xsl:apply-templates select="$up" mode="object.title.markup"/>
                    </div>
                </xsl:if>
                <div class="webhelptitle2">
				  ...
				  <xsl:call-template name="gentext">
					<xsl:with-param name="key" select="'HighlightButton'"/>
                        ...
                        <xsl:if test="count($prev) &gt; 0
                                        ...
                                        or count($next) &gt; 0">
                            <td>
                                <xsl:variable name="has_prev" select="count($prev)&gt;0"/>
                                <xsl:variable name="has_up" select="count($up)&gt;0 and generate-id($up) != generate-id($home)"/>
                                <xsl:if test="$has_prev">
                                    <a accesskey="p">
                                ...
                                <xsl:if test="count($next)>0">
                                    <xsl:if test="$has_prev or $has_up">
                                    |
                                    </xsl:if>
                                    <a accesskey="n">
                                        <xsl:attribute name="href">
    ...
    <xsl:template name="webhelptoc">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                ...
                <xsl:variable name="toc_label_gen">
                    <xsl:call-template name="gentext"><xsl:with-param name="key" select="'WebHelpContents'"/></xsl:call-template>
                </xsl:variable>
                <xsl:variable name="search_label_gen">
                    <xsl:call-template name="gentext"><xsl:with-param name="key" select="'Search'"/></xsl:call-template>
                </xsl:variable>
                <xsl:variable name="toc_label">
                    <xsl:choose>
                        <xsl:when test="not($toc_label_gen)"><xsl:text>Contents</xsl:text></xsl:when>
                        <xsl:otherwise><xsl:value-of select="$toc_label_gen"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="search_label">
                    <xsl:choose>
                        <xsl:when test="not($search_label_gen)"><xsl:text>Search</xsl:text></xsl:when>
                        <xsl:otherwise><xsl:value-of select="$search_label_gen"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="part_nodes" select="/*/part"/>
                <xsl:variable name="current_part" select="ancestor-or-self::part[1]" />

                <div>
                    <div id="leftnavigation">
                        <div id="tabs">
                            <ul id="tocsearchnavigation" style="display:none">
                                <li id="tocnavigationtab">
                                    <a href="#treeDiv"><em><xsl:value-of select="$toc_label"/></em></a>
                                </li>
                                <xsl:if test="$webhelp.include.search.tab != 'false'">
                                    <li id="searchnavigationtab">
                                        <a href="#searchDiv"><em><xsl:value-of select="$search_label"/></em></a>
                                    </li>
                                </xsl:if>
                            </ul>
                            <div id="treeDiv">
                                ...
                                <div id="ulTreeDiv" style="display:none;">
                                    <div id="treenavigationtitle" style="display:none;">
                                        <xsl:value-of select="$toc_label"/>
                                    </div>
                                    <ul id="tree" class="filetree">
                                      <xsl:choose>
                                        <xsl:when test="$current_part">
                                          <xsl:apply-templates select="$current_part/*" mode="webhelptoc">
                                            <xsl:with-param name="currentid" select="$currentid"/>
                                          </xsl:apply-templates>
                                        </xsl:when>
                                        <xsl:when test="$part_nodes">
                                          <xsl:apply-templates select="$part_nodes[1]/*" mode="webhelptoc">
                                            <xsl:with-param name="currentid" select="$currentid"/>
                                          </xsl:apply-templates>
                                        </xsl:when>
                                        <xsl:otherwise>
                            ...
                            <xsl:if test="$webhelp.include.search.tab != 'false'">
                                <div id="searchDiv">
                                    <div id="search">
                                        <form onsubmit="startWebHelpSearch(ditaSearch_Form);return false"
                                              name="ditaSearch_Form"
                                              class="searchForm">
                                            <fieldset class="searchFieldSet">
                                                <legend><xsl:value-of select="$search_label"/></legend>
                                                <div align="center">
                                                    <input id="textToSearch" name="textToSearch" type="text"
                                                           class="searchText"/>
                                                    <xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
                                                    <input onclick="startWebHelpSearch(ditaSearch_Form)" type="button"
                                                           class="searchButton"
                                    ...
                                    <div id="closeSearchResults" style="display:none">
                                        <a href="javascript:closeSearchShowToc();"><xsl:value-of select="$toc_label"/></a>
                                    </div>
                ...
                <xsl:if test="count($part_nodes) != 0">
                  <div id="bookpartsnavigation">
                    <xsl:call-template name="webhelp.parts.navigation" >
                      <xsl:with-param name="partnodes" select="$part_nodes"/>
                      <xsl:with-param name="currentpartid" select="generate-id($current_part)" />
                    </xsl:call-template>
                  </div>
                </xsl:if>
                <xsl:if test="($docma.breadcrumbs = 'true') and count(parent::*)">
            ...
            <xsl:with-param name="content">
                <html>
                    <head>
                        ...
                        <meta http-equiv="Refresh" content="0; URL=content/{$default.topic}"/>
    ...
    <xsl:template name="webhelp.parts.navigation">
        <xsl:param name="partnodes"/>
        <xsl:param name="currentpartid"/>
        
        <xsl:variable name="firstpartid" select="generate-id($partnodes[1])"/>
        <ul>
        <xsl:for-each select="$partnodes" >
          <xsl:variable name="title">
            <xsl:if test="$webhelp.autolabel=1">
                <xsl:variable name="label.markup">
                    <xsl:apply-templates select="." mode="label.markup"/>
          ...
          <li>
            <xsl:if test="(($currentpartid = '') and ($partid = $firstpartid)) or ($partid = $currentpartid)">
                <xsl:attribute name="class">book-currentpart</xsl:attribute>
            </xsl:if>
            <a href="{substring-after($href,concat($webhelp.base.dir,'/content/'))}">
                <xsl:value-of select="$title"/>
            </a>
