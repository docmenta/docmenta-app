...
<xsl:template match="orderedlist">
  ...
  <div>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <table class="ol-table" border="0" cellspacing="0">
          <colgroup>
            <col align="{$direction.align.start}" valign="top"/>
            <col valign="top"/>
          </colgroup>
          ...
          <xsl:if test="@spacing='compact'"><xsl:message><xsl:text>Compact spacing via @spacing attribute cannot be set in strict XHTML output for listitem: </xsl:text><xsl:value-of select="@spacing"/></xsl:message></xsl:if>
          <xsl:apply-templates mode="docma-ol-table" select="listitem                         |comment()[preceding-sibling::listitem]                         |processing-instruction()[preceding-sibling::listitem]"/>
        </table>
      </xsl:otherwise>
    </xsl:choose>
  </div>
...
<xsl:template match="orderedlist/listitem" mode="docma-ol-table">
  <tr>
    <td class="ol-td-number" valign="top">
      <xsl:choose>
        <xsl:when test="(@override = 'none') or (@override = 'NONE')"><xsl:text> </xsl:text></xsl:when>
        <xsl:otherwise>
          <xsl:apply-templates select="." mode="item-number"/>
        </xsl:otherwise>
      </xsl:choose>
    </td>
    <td class="ol-td-content" valign="top">
      <xsl:choose>
        <xsl:when test="$show.revisionflag != 0 and @revisionflag">
          <div class="{@revisionflag}">
            <xsl:apply-templates/>
