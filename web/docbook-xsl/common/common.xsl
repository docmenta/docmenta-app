...
<xsl:template name="orderedlist-item-number">
  ...
  <xsl:choose>
    ...
    <xsl:when test="$node/preceding-sibling::listitem[not(@override='none')]">
      <xsl:variable name="pnum">
        <xsl:call-template name="orderedlist-item-number">
          <xsl:with-param name="node" select="$node/preceding-sibling::listitem[not(@override='none')][1]"/>
        </xsl:call-template>
      </xsl:variable>
...
<xsl:template match="orderedlist/listitem" mode="item-number">
  ...
  <xsl:if test="not($item-number='none')">
