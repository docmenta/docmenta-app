...
<xsl:template match="figure|table|example" mode="label.markup">
  ...
  <xsl:variable name="roleval" select="@role" />
    ...
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="$prefix != ''">
          ...
          <xsl:choose>
            <xsl:when test="self::example[@role]">
              <xsl:number format="1" from="chapter|appendix" count="example[@role=$roleval]" level="any" />
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test="self::example[@role]">
              <xsl:number format="1" from="book|article" count="example[@role=$roleval]" level="any"/>
            </xsl:when>
            <xsl:otherwise>
