...
function insertTable() {
	...
	cellspacing = formObj.elements['cellspacing'].value != "" ? formObj.elements['cellspacing'].value : "0";
    ...
    if (formObj.elements['long_table'].checked) {
      if (className.length == 0) className = 'keep_together_auto';
      else className += ' keep_together_auto';
    }
...
function init() {
    ...
    var cls_arr = className.split(' ');
    var keep_idx = -1;
    className = '';
    for (var i=0; i < cls_arr.length; i++) {
      if (cls_arr[i] == 'keep_together_auto') keep_idx = i;
      else {
        if (className.length == 0) className = cls_arr[i];
        else className += ' ' + cls_arr[i];
      }
    }
    formObj.long_table.checked = (keep_idx >= 0);
