...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="table_template">
    ...
    <xsl:call-template name="create_colgroup" ><xsl:with-param name="row_num" select="1" /></xsl:call-template>
  ...
  <xsl:template name="create_colgroup">
    ...
    <!-- <xsl:message>Row number: <xsl:value-of select="$row_num" /> Widths: <xsl:value-of select="$old_widths" /></xsl:message> -->
    <xsl:variable name="row_count" select="count(descendant::tr)" />
    <xsl:if test="$row_num &lt;= $row_count" >
      <xsl:variable name="row" select="(descendant::tr)[position() = $row_num]" />
      ...
      <xsl:variable name="widths"><xsl:call-template name="merge_col_widths" >
          <xsl:with-param name="widths1" select="$old_widths" />
          <xsl:with-param name="widths2" select="$current_widths" />
      </xsl:call-template></xsl:variable>
      <xsl:choose>
        <xsl:when test="not(contains($widths, '?')) or ($row_num &gt;= $row_count)">
          <colgroup>
  ...
  <xsl:template name="get_col_widths">
    ...
    <xsl:for-each select="$row/*[(local-name() = 'td') or (local-name() = 'th')]" >
      ...
      <xsl:if test="@colspan &gt; 1" >
        <xsl:call-template name="write_no_colwidth" ><xsl:with-param name="span_cnt" select="@colspan - 1" /></xsl:call-template>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="write_no_colwidth">
    <xsl:param name="span_cnt" />

    <xsl:text>?,</xsl:text>
    <xsl:if test="$span_cnt &gt; 1" >
      <xsl:call-template name="write_no_colwidth" ><xsl:with-param name="span_cnt" select="$span_cnt - 1" /></xsl:call-template>
    </xsl:if>
  </xsl:template>


  <xsl:template name="merge_col_widths">
    <xsl:param name="widths1" />
    <xsl:param name="widths2" />

    <xsl:variable name="width_1" select="normalize-space(substring-before($widths1, ','))" />
    <xsl:variable name="width_2" select="normalize-space(substring-before($widths2, ','))" />
    <xsl:if test="(string-length($width_1) &gt; 0) or (string-length($width_2) &gt; 0)" >
      <xsl:choose>
        <xsl:when test="(string-length($width_1) &gt; 0) and ($width_1 != '?')" ><xsl:value-of select="$width_1" /></xsl:when>
        <xsl:otherwise><xsl:value-of select="$width_2" /></xsl:otherwise>
      </xsl:choose>
      <xsl:text>,</xsl:text>
      <xsl:call-template name="merge_col_widths" >
        <xsl:with-param name="widths1" select="substring-after($widths1, ',')" />
        <xsl:with-param name="widths2" select="substring-after($widths2, ',')" />
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
